...
from . import msc_configs as configs
from .msc_resnet import ResNetV2
...
class Encoder(nn.Module):
    def __init__(self, config, vis, num_layers=3):# num_layers 12 -> 3
...
class Transformer(nn.Module):
    def __init__(self, config, img_size, vis):
        ...
        self.encoder_feartures = nn.ModuleList([
            Encoder(config, vis, num_layers) for num_layers in [3, 3, 3]
        ])
...
class VisionTransformer(nn.Module):
    def __init__(self, config, img_size=224, num_classes=21843, zero_head=False, vis=False):
        ...
        self.layer = nn.Sequential(
            nn.Conv1d(in_channels=196, out_channels=1, kernel_size=1, stride=1),
        ...
        self.pro_decoder = nn.Sequential(
            nn.Linear(prev_dim_decoder, prev_dim_decoder, bias=False),
            nn.BatchNorm1d(prev_dim_decoder),
            nn.ReLU(inplace=True),
            nn.Linear(prev_dim_decoder, prev_dim_decoder, bias=False),
            nn.BatchNorm1d(prev_dim_decoder),
            nn.ReLU(inplace=True),
            nn.Linear(prev_dim_decoder, prev_dim_decoder, bias=False),
            nn.BatchNorm1d(prev_dim_decoder, affine=False)
        )
        ...
        self.pred_decoder = nn.Sequential(
            nn.Linear(in_features=prev_dim_decoder, out_features=prev_dim_decoder, bias=False),
            nn.BatchNorm1d(prev_dim_decoder),
            nn.ReLU(inplace=True),
            nn.Linear(prev_dim_decoder, prev_dim_decoder)
        )

    def forward(self, x, x_aug):
        if x.size()[1] == 1:
            x = x.repeat(1, 3, 1, 1)
            x_aug = x_aug.repeat(1, 3, 1, 1)

        x, encoded_ms, attn_weights, features = self.transformer(x)  # (B, n_patch, hidden)
        x_aug, _, _, _ = self.transformer(x_aug)  # (B, n_patch, hidden)

        x_aug_2 = x_aug
        x_o = x
        x_o = self.layer(x_o)
        x_aug = self.layer(x_aug)
        p_o = self.pro(x_o[:, 0, :])
        p_a = self.pro(x_aug[:, 0, :])
        ...
        x_aug_2 = self.decoder(x_aug_2, features)
        logits2 = self.segmentation_head(x_aug_2)

        return logits1, logits2, p_o.detach(), p_a.detach(), z_o, z_a
